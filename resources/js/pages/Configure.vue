    ...
    <div>
        ...
        <card class="p-8 space-y-4">
            <template v-if="resource">
                <p v-if="resource">Use modifiers to modify the value <i>before</i> it gets saved to your resource.</p>

                <p>You can drag and drop modifiers to re-order them.</p>
                ...
                <table cellpadding="10">
                    ...
                    <tbody v-if="fields && fields.length">
                        <tr v-for="field in fields" class="border-b">
                            <td class="pr-2">
...
export default {
    ...
    watch: {
        resource: {
            async handler(newValue) {
                ...
                await this.fetchFields(newValue);
                ...
                for (let { name, attribute } of this.fields) {
    ...
    mounted() {
        if (this.resource) {
            this.fetchFields(this.resource);
        }
    },
    ...
    methods: {
        ...
        fetchFields(resource) {
            return Nova.request()
                .post(route('csv_import.resource.creation_fields.index', { resource: resource }, false), {})

                .then((response) => {
                    this.fields = response.data;
                })
                .catch((e) => {
                    Nova.error('There was a problem fetching fields');
                });
        },
